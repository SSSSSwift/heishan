...
function My_ChuangSong_List()
	...
	for i = 0, JY.SceneNum-1 do
		...
		or i == 108 or i == 109 or i == 110 or i == 111 or i == 113 or i == 114 or i == 116 or i == 117 or i == 104 or i == 119  or i == 121 or i == 122 then
...
function My_ChuangSong_Ex()     
	...
	local btn = {"指点江山", "激扬文字"};
	local num = #btn;
	local r = JYMsgBox(title,str,btn,num,122,1);
	if r == 1 then
	...
	elseif r == 2 then
		...
		if sid ~= nil then			
			...
			or sid == 108 or sid == 109 or sid == 110 or sid == 111 or sid == 113 or sid == 114  or sid == 121 or sid == 122 or sid == 104 or JY.Scene[sid]["进入条件"] == 1 then
...
--山洞女主角的剧情
function mm4R()
	local r = JYMsgBox("请选择", "你喜欢哪种类型的武功？", {"拳法","指法","剑法","刀法","奇门"}, 5, 384)
	if r == 1 then
	...
	local r = JYMsgBox("请选择", "内力性质呢？", {"阴内","阳内","调和"}, 3, 384)
	if r == 1 then
		JY.Person[92]["内力性质"] = 0
		Cls()  --清屏
	elseif r == 2 then
...
OEVENTLUA[30055] = function() --遇大侠
	local money = math.random(3) * 1500
	local head = math.random(350, 360)
	Cls()
...
OEVENTLUA[30056] = function() --遇铁匠
	local money = math.random(3) * 5000
	local head = math.random(350, 360)
	Cls()
...
OEVENTLUA[30057] = function(aa, bb) --洗武功
	...
	JY.Person[0]["武功等级1"] = 900 --math.random(750, 900) 武骧金星：取消这个没有意义的设置
	Cls()
	dark()
	light()
